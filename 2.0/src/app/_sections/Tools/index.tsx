...
export default function ToolsSection() {
  return (
    <section className="relative">
      <BlurFade inView className="flex justify-center w-full max-w-screen-lg p-8 sm:p-16 mx-auto">
            ...
            <div className="flex items-center justify-around w-[200%] min-w-fit carousel-animation hover:paused">
              {tools.map((tool) => (
                <Link
                  ...
                  href={tool.href}
                  target="_blank"
                  className="transition-transform block relative group overflow-visible"
                >
                  {tool.image}
                  <span className="absolute -top-8 left-1/2 -translate-x-1/2 text-background py-1 px-2 rounded-sm opacity-0 transition-all w-max group-hover:opacity-100 group-hover:-top-9 bg-foreground text-sm">
                    {tool.name}
                  </span>
                </Link>
              ))}
              {tools.map((tool) => (
                <Link
                  ...
                  href={tool.href}
                  target="_blank"
                  className="transition-transform block relative group"
                >
                  {tool.image}
                  <span className="absolute -top-8 left-1/2 -translate-x-1/2 text-background py-1 px-2 rounded-sm opacity-0 transition-all w-max group-hover:opacity-100 group-hover:-top-9 bg-foreground text-sm">
                    {tool.name}
